...
All captures, plots, and downloadable files go to `MCP_SHARED_DATA_DIR` (env var; default `/home/worker/uhd-mcp/shared-data-layer`). `list_shared_files` and `download_file` operate only within this dir — `download_file` enforces a `commonpath` containment check to prevent path escape. Preserve that check when touching file-serving code.
...
Built `FROM` an internal OpenShift registry `uhd:4.7` base, adds GNU Radio + pipx, creates a non-root `worker` user (UID/GID `1002220000`), `git clone`s the repo at build-time (`--branch ${GIT_REF}`, default `develop`, via SSH mount), runs `./setup.sh`, and uses `CMD ["./start.sh"]`. Because the repo is baked in at build time, the running container relies on `start.sh` → `update-repo.sh` (gated on `PAT_TOKEN`) to pull newer commits on each restart without rebuilding the image.

### OpenShift/Kubernetes — [deploy/](deploy/)
Target namespace is `mcp-services`. Apply order (see [deploy/README.md](deploy/README.md)):
...
Topology:
- **Deployment** runs `./start.sh` (server on container port 8080), mounts PVC `mcp-shared-data-layer-pvc` at the shared data dir, and runs locked-down (non-root UID `1002220000`, all caps dropped, `RuntimeDefault` seccomp) — matching the Dockerfile's `worker` user.
